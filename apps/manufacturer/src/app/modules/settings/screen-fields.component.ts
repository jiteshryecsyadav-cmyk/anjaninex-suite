  ...
  template: `
    <div class="max-w-5xl mx-auto">
      ...
      <div class="flex items-start justify-between mb-4 gap-3 flex-wrap">
        <div>
          <h2 class="font-display font-black text-2xl text-[#1B2E5C]">Screen &amp; Fields</h2>
      ...
      @for (s of screens; track s.key) {
        <div class="bg-white rounded-xl border mb-2 overflow-hidden">
          <button (click)="toggle(s.key)"
                  class="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-anjaninex-navy-soft">
            <span class="font-bold text-[#1B2E5C]">
              <span class="inline-block w-4 text-[#DC2626]">{{ isOpen(s.key) ? '▾' : '▸' }}</span>
              {{ s.name }}
            </span>
