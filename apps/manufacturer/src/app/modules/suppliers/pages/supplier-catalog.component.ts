...
export class SupplierCatalogComponent {
  ...
  ngOnInit() { if (this.supplierId) this.load(); }

  img(url: string) { return url.startsWith('http') ? url : `${environment.apiUrl}${url}`; }

  load() {
    this.loading.set(true);
    this.svc.getCatalog(this.supplierId).subscribe({
  ...
  addVariety() {
    ...
    this.svc.addVariety(this.supplierId, { categoryId, categoryName, name, dNo: this.newDno.trim() || null })
      .subscribe({ next: () => { this.newName = ''; this.newDno = ''; this.newCatName = ''; this.load(); } });
  }

  delVariety(v: Variety) {
    if (!confirm(`Variety "${v.name}" delete karein?`)) return;
    this.svc.deleteVariety(v.id).subscribe({ next: () => this.load() });
  }

  addRate(v: Variety) {
    const rate = this.rateInput[v.id];
    if (rate == null || isNaN(rate)) return;
    this.svc.addVarietyRate(v.id, { rate, unit: this.unitInput[v.id] || 'mtr' })
      .subscribe({ next: () => { this.rateInput[v.id] = null; this.load(); } });
  }
  delRate(v: Variety, id: string) { this.svc.deleteVarietyRate(id).subscribe({ next: () => this.load() }); }

  onPhoto(v: Variety, ev: Event) {
    const input = ev.target as HTMLInputElement;
    const file = input.files?.[0];
    if (!file) return;
    this.svc.uploadVarietyPhoto(v.id, file).subscribe({ next: () => { input.value = ''; this.load(); } });
  }
  delPhoto(v: Variety, id: string) { this.svc.deleteVarietyPhoto(id).subscribe({ next: () => this.load() }); }
}
