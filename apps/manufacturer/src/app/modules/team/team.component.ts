...
@Component({
  ...
  template: `
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex items-center justify-between mb-4">
        <div>
          <h2 class="font-display font-black text-2xl text-[#1B2E5C]">🛡️ Team &amp; Security</h2>
          <p class="text-sm text-[#4A5878]">Staff logins · Roles · Permissions · Branches</p>
        </div>
      </div>
      ...
      @if (tab() === 'users') {
        <div class="card mb-4">
          <div class="flex items-center justify-between mb-3">
            <h3 class="font-bold text-[#1B2E5C]">Staff Logins</h3>
            <button (click)="openUserForm()" class="btn-primary text-sm">+ Add User</button>
          </div>
          <table class="w-full text-sm">
            <thead class="bg-[#f0e6ff] text-[#1B2E5C] text-xs uppercase">
              <tr>
                ...
                <tr class="border-t hover:bg-[#faf5ff]">
                  ...
                  <td class="px-2 py-2">
                    @for (r of u.roles; track r.id) {
                      <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#ede9fe] text-[#1B2E5C] mr-1">{{ r.name }}</span>
                    }
                  </td>
                  ...
                  <td class="px-2 py-2 text-center whitespace-nowrap">
                    <button (click)="openUserForm(u)" class="text-xs font-bold text-[#1B2E5C] hover:underline mr-2">✏️ Edit</button>
      ...
      @if (tab() === 'roles') {
        <div class="card mb-4">
          <div class="flex items-center justify-between mb-3">
            <h3 class="font-bold text-[#1B2E5C]">Roles</h3>
      ...
      @if (tab() === 'perms') {
        <div class="card mb-4">
          <div class="flex items-center justify-between mb-1 flex-wrap gap-2">
            <div>
              <h3 class="font-bold text-[#1B2E5C]">Menu &amp; screen CRUD</h3>
          ...
          @if (!permRoleId) {
            ...
            <div class="flex gap-2 my-3">
              <button (click)="allRead(true)" class="px-3 py-1.5 rounded-lg text-xs font-bold bg-[#ede9fe] text-[#1B2E5C] hover:bg-[#ddc8f5]">All Read on</button>
              <button (click)="allRead(false)" class="px-3 py-1.5 rounded-lg text-xs font-bold bg-white border border-[#ddc8f5] text-[#1B2E5C] hover:bg-[#f0e6ff]">All Read off</button>
            ...
            <table class="w-full text-sm">
              <thead class="bg-[#f0e6ff] text-[#1B2E5C] text-xs uppercase">
                <tr>
                  ...
                  <th class="px-2 py-2 text-center w-14 text-[#1B2E5C]">✓ All</th>
                </tr>
              </thead>
              <tbody>
      ...
      @if (tab() === 'branches') {
        <div class="card mb-4">
          <div class="flex items-center justify-between mb-3">
            <h3 class="font-bold text-[#1B2E5C]">Branches</h3>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              @for (b of branches(); track b.id; let i = $index) {
                <tr class="border-t hover:bg-[#faf5ff]">
                  <td class="px-2 py-2 font-mono text-xs">{{ i + 1 }}</td>
                  <td class="px-2 py-2 font-mono font-bold text-[#1B2E5C]">{{ b.code }}</td>
                  ...
                  <td class="px-2 py-2 text-center whitespace-nowrap">
                    <button (click)="openBranchForm(b)" class="text-xs font-bold text-[#1B2E5C] hover:underline mr-2">✏️ Edit</button>
                    @if (!b.isHeadOffice) {
      ...
      @if (showUserForm()) {
        <div class="modal-bg" (click)="showUserForm.set(false)">
          <div class="modal-card" (click)="$event.stopPropagation()">
            <h3 class="font-bold text-lg text-[#1B2E5C] mb-4">{{ editUserId ? '✏️ Edit User' : '+ Naya User' }}</h3>
            @if (error()) {
              <div class="bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded text-sm mb-3">{{ error() }}</div>
            }
      ...
      @if (pwUser()) {
        <div class="modal-bg" (click)="pwUser.set(null)">
          <div class="modal-card" style="max-width:420px" (click)="$event.stopPropagation()">
            <h3 class="font-bold text-lg text-[#1B2E5C] mb-3">🔒 Password Reset — {{ pwUser()!.fullName }}</h3>
      ...
      @if (showRoleForm()) {
        <div class="modal-bg" (click)="showRoleForm.set(false)">
          <div class="modal-card" style="max-width:460px" (click)="$event.stopPropagation()">
            <h3 class="font-bold text-lg text-[#1B2E5C] mb-3">+ Naya Role</h3>
            <div class="grid grid-cols-2 gap-3">
      ...
      @if (showBranchForm()) {
        <div class="modal-bg" (click)="showBranchForm.set(false)">
          <div class="modal-card" (click)="$event.stopPropagation()">
            <h3 class="font-bold text-lg text-[#1B2E5C] mb-4">{{ editBranchId ? '✏️ Edit Branch' : '+ Nayi Branch' }}</h3>
            <div class="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                @if (pinLoading()) { <small class="text-[#1B2E5C]">🔎 dhundh raha hoon…</small> }
  ...
  `,
  styles: [`
    .lbl2 { display:block; font-size:10px; font-weight:800; color:#4A5878; text-transform:uppercase; letter-spacing:.4px; margin-bottom:3px; }
...
export class TeamComponent {
  ...
  tabCls(t: Tab): string {
    return this.tab() === t
      ...
      : 'px-4 py-2 rounded-lg text-sm font-semibold bg-white border border-[#ddc8f5] text-[#1B2E5C] hover:bg-[#f0e6ff]';
  }
