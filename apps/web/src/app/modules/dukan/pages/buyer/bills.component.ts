...
@Component({
  ...
  template: `
  @if (sel(); as o) {
    <button class="btn ghost sm no-print" style="margin-bottom:12px" (click)="sel.set(null)">← Back to Bills</button>
    <div class="card printable" style="padding:26px;max-width:640px">
      ...
      <div style="margin:14px 0;font-size:13px">
        <div style="font-weight:700;margin-bottom:2px">Ship to:</div>
        <div>{{ o.receiver }}</div>
  ...
  } @else if (ds.orders().length === 0) {
    <div class="empty card" style="padding:50px">Abhi koi bill nahi 🧾</div>
  } @else {
    <h2 style="font-size:18px;margin-bottom:14px">Tax Invoices</h2>
    <div class="card" style="padding:6px 16px">
      <table class="tbl">
        <tr><th>Bill No</th><th>Order</th><th>Date</th><th>Amount</th><th></th></tr>
        @for (o of ds.orders(); track o.id) {
          <tr>
            <td><b>{{ o.billNo }}</b></td><td>{{ o.id }}</td>
